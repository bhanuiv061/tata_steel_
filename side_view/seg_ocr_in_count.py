...
def make_ocr_variants(raw_crop):
    ...
    eq3    = cv2.cvtColor(eq, cv2.COLOR_GRAY2BGR)
    variants.append(("EQGRAY", eq3))
    ...
    variants.append(("THRESH", thresh3))

    inv     = cv2.bitwise_not(thresh)
    inv3    = cv2.cvtColor(inv, cv2.COLOR_GRAY2BGR)
    variants.append(("THRESH_INV", inv3))

    return variants

def run_easyocr_on_crop(img, label=""):
    results = ocr_reader.readtext(
        img,
        detail=1,
        paragraph=False,
        allowlist="0123456789OIlSBZGTQo",
        width_ths=0.8,
        contrast_ths=0.05,
        adjust_contrast=0.7,
    )
    ...
    kept = [(txt, conf) for (_, txt, conf) in results if conf > 0.2]
    if not kept:
        return "", 0.0, []

    joined    = "".join(t for t, c in kept)
    best_conf = max(c for t, c in kept)
    return joined, best_conf, results

def extract_text_easyocr(raw_crop, debug_dir=None, tid=None, frame_no=None):
    ...
    for label, img in variants:
        ...
        raw_text, conf, results = run_easyocr_on_crop(img, label)
...
def crossed_line(prev_side, curr_side, axis):
    ...
    if prev_side == curr_side:
        return None
    if axis == "x":
        # REMOVED: "in" direction (left → right)
        if prev_side == "right" and curr_side == "left":  return "out"
    else:
        # REMOVED: "in" direction (top → bottom)
...
def draw_count_hud(frame, count_out, class_order):
    ...
    cv2.putText(frame, "OUT",   (COL_OUT,  hy), FONT, 0.46, (160,160,160), 1, cv2.LINE_AA)
...
def run(weights, source, imgsz=640, conf_thres=0.25, iou_thres=0.45,
    ...
    tracker   = Sort()
    count_out = {v: 0 for v in names.values()}   # removed count_in
    ...
    for data in tqdm(dataset):
        ...
        for trk in tracks.astype(int):
            ...
            # REMOVED: count_in increment — only count_out is tracked
            if direction == "out":
                count_out[cls_name] += 1
                print(f"⬅  OUT | {cls_name} | TID:{tid} | frame:{frame_no}")

                # ✅ Confirm OCR ONLY AFTER crossing
            ...
            if OCR_CLASS_NAME and cls_name == OCR_CLASS_NAME and tid not in ocr_done:
                ...
                if number:
                    ...
                    if n >= MIN_OCR_FRAMES:
                        ...
                        if count >= 2:
                            final = most_common
                            ocr_temp[tid] = final   # store temporarily
                            ocr_done[tid] = True    # stop further OCR
        ...
        draw_ocr_panel(frame, ocr_results, ocr_candidates, ocr_done, names, OCR_CLASS_NAME)   # removed count_in arg

        cv2.putText(frame, f"frame:{frame_no}", (w - 130, h - 10),
    ...
    save_summary_json(save_dir, count_out, ocr_results)   # removed count_in arg
    ...
    print(f"  └─ ocr_debug_crops/        raw crops + all 5 variants per attempt")
    print("=" * 60)
...
#         clean = normalise_ocr_text(txt)

#         # ✅ Reject weak / short garbage
#         if len(clean) < 8:
#             continue
